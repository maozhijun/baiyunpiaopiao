...
//            $this->channels[] = '火猫直播4##1525914';
    ...
    {
        $ets = EncodeTask::query()->where('from', env('APP_NAME'))->where('to', 'HuoMao')->where('created_at', '>', date_create('-48 hour'))->whereIn('status', [1, 2, -1])->get();
...
//            $channel = $request->input('channel');
            $channel = $request->input('channel');
            $ets = EncodeTask::query()->where('from', env('APP_NAME'))->where('to', 'HuoMao')->where('created_at', '>', date_create('-48 hour'))->whereIn('status', [1, 2, -1])->inRandomOrder()->get();
