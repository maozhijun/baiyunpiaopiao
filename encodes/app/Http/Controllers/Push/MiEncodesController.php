...
//            $this->channels[] = '小米直播8##cid201803241148222057579';
    ...
    {
        $ets = EncodeTask::query()->where('from', env('APP_NAME'))->where('to', 'Mi')->where('created_at', '>', date_create('-48 hour'))->whereIn('status', [1, 2, -1])->get();
    ...
    public function created(Request $request)
    {
        if ($request->isMethod('post')
            ...
            $ets = EncodeTask::query()->where('from', env('APP_NAME'))->where('to', 'Mi')->where('created_at', '>', date_create('-48 hour'))->whereIn('status', [1, 2, -1])->inRandomOrder()->get();
