...
class NeteaseEncodesController extends BaseController
{
    ...
    {
        $ets = EncodeTask::query()->where('from', env('APP_NAME'))->where('to', 'Netease')->where('status', '>=', 1)->get();
    ...
    public function created(Request $request)
    {
        if ($request->isMethod('post')
            ...
            $live_m3u8_url = 'http://hls2.live.126.net/live/' . explode('?', $roomId)[0] . '.m3u8';//m3u8地址
