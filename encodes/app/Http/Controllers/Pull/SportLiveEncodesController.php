...
class SportLiveEncodesController extends BaseController
{
    private $channels = [];
    ...
    public function index(Request $request)
    {
        ...
        return view('manager.pull.sportlive', ['lives' => $lines, 'ets' => $ets, 'channels' => $this->channels]);
    }
...
//                $urls['m3u8_' . $name] = $url;
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://mediaonsport.de/tv/sBLUE/api.php?latest=250');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
        curl_setopt($ch, CURLOPT_USERAGENT, "Football%20Live/1.4 CFNetwork/894 Darwin/17.4.0");
...
//        $data = array(
//            'json='.urlencode(json_encode(['videolive' => $id])),
//        );
//        dump($data);
