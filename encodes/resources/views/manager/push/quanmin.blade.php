...
@section('content')
    <form action="/manager/quanmin/created/" method="post">
        {{ csrf_field() }}
        ...
        <table class="table table-striped">
            ...
            @foreach($ets as $et)
                <tr>
                    ...
                    <td>
                        @if($et->status == 1)
                            <a class="btn btn-xs btn-danger" href="javascript:if(confirm('确认删除')) location.href='/manager/quanmin/stop/{{ $et->id }}'">停止</a>
                        @endif
                    </td>
                </tr>
            @endforeach
