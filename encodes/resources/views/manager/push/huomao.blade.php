...
@section('content')
    <form action="/manager/huomao/created/" method="post">
        {{ csrf_field() }}
        ...
        <table class="table table-striped">
            ...
            @foreach($ets as $et)
                <tr>
                    ...
                    <td><a target="_blank" href="https://www.huomao.com/{{ explode('##',$et->channel)[1] }}">{{ explode('?',$et->channel)[0] }}</a></td>
                    <td width="50%">
                    ...
                    <td>
                        @if($et->status != 0)
                            <a class="btn btn-xs btn-danger"
                               href="javascript:if(confirm('确认删除')) location.href='/manager/huomao/stop/{{ $et->id }}'">停止</a>
                            <br><br>
                            <a class="btn btn-xs btn-warning"
                               href="javascript:if(confirm('确认重推')) location.href='/manager/huomao/repeat/{{ $et->id }}'">重推</a>
                        @endif
                    </td>
                </tr>
            @endforeach
