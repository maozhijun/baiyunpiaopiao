...
function setOnscroll (){
    UA = myBrowser();
    if (document.body.offsetWidth < 1400 && (UA == "WebKit" || UA == "FF")) {
        window.onscroll = function () {
            var LeftBar = document.getElementById('LeftBar');
            LeftBar.style.top = ((document.documentElement.scrollTop || document.body.scrollTop)*1.25 + 100) + 'px';
        }
        LeftBar.style.bottom = 0;
        LeftBar.style.top = '100px';
        LeftBar.style.transition = 'top 0.1s';
    }
}
...
function setBG () {
    ...
    $('#Match_Player table tbody td').mouseover(function(){
        setTB(this);
    })
    $('#Match_Player table').mouseout(function(){
        cleanTB(this);
    })
}
