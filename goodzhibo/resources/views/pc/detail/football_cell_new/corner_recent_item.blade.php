...
<div class="Border" id="Corner_Data_{{$key}}_Content" league="0" ha="0">
    @component('pc.detail.football_cell_new.corner_recent_item_content',['teamId'=>('H' == $key ? $match['hid'] : $match['aid']),'teamName'=>$name,'key'=>$key,'data'=>$data['all'],'league'=>0,'ha'=>0,'analyse'=>$data['statistic']['all']])
    @endcomponent
    @component('pc.detail.football_cell_new.corner_recent_item_content',['teamId'=>('H' == $key ? $match['hid'] : $match['aid']),'teamName'=>$name,'key'=>$key,'data'=>$data['sameHA'],'league'=>0,'ha'=>1,'analyse'=>$data['statistic']['sameHA']])
    @endcomponent
    @component('pc.detail.football_cell_new.corner_recent_item_content',['teamId'=>('H' == $key ? $match['hid'] : $match['aid']),'teamName'=>$name,'key'=>$key,'data'=>$data['sameL'],'league'=>1,'ha'=>0,'analyse'=>$data['statistic']['sameL']])
    @endcomponent
