@if(isset($odd) && (isset($odd['middle1']) || isset($odd['middle2'])))
    <div class="odd default" id="Corner_Odd">
        ...
        <table>
            ...
            <tr>
                ...
                @if($odd['up2'] > $odd['up1'])
                    class="red"
                @elseif($odd['up2'] < $odd['up1'])
                    class="green"
                @endif
                >{{$odd['up2']}}</td>
                <td
                        @if($odd['middle2'] > $odd['middle1'])
                        class="red"
                        @elseif($odd['middle2'] < $odd['middle1'])
                        class="green"
                        @endif
                >{{$odd['middle2']}}</td>
                <td
                        @if($odd['down2'] > $odd['down1'])
                        class="red"
                        @elseif($odd['down2'] < $odd['down1'])
                        class="green"
                        @endif
                >{{$odd['down2']}}</td>
            </tr>
...
@if((count($historyBattle['nhnl']) + count($historyBattle['shnl']) + count($historyBattle['nhsl']) + count($historyBattle['shsl'])) > 0)
    <div class="battle default" id="Corner_Battle" league="0" ha="0">
        ...
        @component('pc.detail.football_cell.corner_history_item',['hid'=>$match['hid'],'league'=>0,'ha'=>0,'hname'=>$hname,'aname'=>$aname,'data'=>$historyBattle['nhnl'],'analyse'=>$historyBattleResult['all']])
        @endcomponent
        @component('pc.detail.football_cell.corner_history_item',['hid'=>$match['hid'],'league'=>1,'ha'=>0,'hname'=>$hname,'aname'=>$aname,'data'=>$historyBattle['nhsl'],'analyse'=>$historyBattleResult['league']])
        @endcomponent
        @component('pc.detail.football_cell.corner_history_item',['hid'=>$match['hid'],'league'=>0,'ha'=>1,'hname'=>$hname,'aname'=>$aname,'data'=>$historyBattle['shnl'],'analyse'=>$historyBattleResult['team']])
        @endcomponent
        @component('pc.detail.football_cell.corner_history_item',['hid'=>$match['hid'],'league'=>1,'ha'=>1,'hname'=>$hname,'aname'=>$aname,'data'=>$historyBattle['shsl'],'analyse'=>$historyBattleResult['both']])
        @endcomponent
    </div>
@endif
@if(isset($recentBattle))
    <div class="history default" id="Corner_History">
        ...
        @if(isset($recentBattle['home']))
            @component('pc.detail.football_cell.corner_recent_item',['key'=>'H','name'=>$hname,'data'=>$recentBattle['home'],'match'=>$match])
            @endcomponent
        @endif
        @if(isset($recentBattle['away']))
            @component('pc.detail.football_cell.corner_recent_item',['key'=>'A','name'=>$aname,'data'=>$recentBattle['away'],'match'=>$match])
            @endcomponent
        @endif
    </div>
@endif
