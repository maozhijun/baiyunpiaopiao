...
@endsection
@section('css')
<link rel="stylesheet" type="text/css" href="{{env('CDN_URL')}}/css/videoList.css">
@endsection
@section('banner')
<div id="Navigation">
    <div class="banner">
        ...
        <div class="default">
            ...
            @foreach($match_array as $match)
                <a href="{{str_replace('https://', 'http://', asset('m/live/'.($match['sport'] == 1 ? 'football':'basketball').'/' . $match['mid'].'.html'))}}">
                    ...
                    <p class="vs">@if($match['isMatching'])<img src="/img/icon_live.png">@else VS @endif</p>
                    <p class="team away">{{$match['aname']}}</p>
                </a>
            @endforeach
        </div>
    @endforeach
    <p id="PC">更多赛事线路与特色功能，请登录电脑版！<br/>www.goodzhibo.com<button class="close" onclick="this.parentNode.style.display='none'"></button></p>
@endsection
@section('js')
