...
@section('css')
    <link rel="stylesheet" type="text/css" href="{{env('CDN_URL')}}/css/videoPhone.css">
@endsection
@section('banner')
    <div id="Navigation">
        <div class="banner"><a class="home" @if($match['sport'] == 1) href="/m/lives.html" @else href="/m/basketball.html" @endif ></a>比赛直播</div>
    </div>
@endsection
@section('content')
    <div class="default" id="Info">
        <div class="team host">
            <img src="{{$host_icon}}" onerror='this.src="{{env('CDN_URL')}}/img/icon_team_default.png"' >
            <p>{{$match['hname']}}</p>
        </div>
        <div class="score">
            @if($show_live || $match['status'] == -1)
            <p style="display: {{$show_live ? 'none' : 'block'}};">
                <span class="host">{{empty($match['hscore']) ? 0 : $match['hscore']}}</span>
                <span class="away">{{empty($match['ascore']) ? 0 : $match['ascore']}}</span>
            </p>
        ...
        <div class="team away">
            <img src="{{$away_icon}}" onerror='this.src="{{env('CDN_URL')}}/img/icon_team_default.png"' >
            <p>{{$match['aname']}}</p>
        </div>
    </div>
