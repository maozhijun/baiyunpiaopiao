...
class MatchLiveChannel extends Model
{
    ...
    const kPlayerAuto = 1, kPlayerIFrame = 11, kPlayerCk = 12, kPlayerM3u8 = 13, kPlayerFlv = 14, kPlayerRTMP = 15, kPlayerExLink = 16, kPlayerClappr = 17;//播放方式，1、自动播放，11、iFrame嵌入。12、ck播放器播放，13、M3U8，14、FLV，15、RTMP，16、跳转外链,17、Clappr播放器
    const kAutoSpider = 1, kAutoHand = 2;//1、爬虫获取的，2、手动录入的。
    const kShow = 1, kHide = 2;//1、显示该直播链接，0、不显示直播链接
    const kIsPrivate = 1, kIsNotPrivate = 2;//1、无版权，2、有版权
...
//        $type = $this->type;
//        $array = ['name'=>$this->name, 'player'=>$this->player, 'channelId'=>$this->id, 'use'=>$this->use, 'pri'=>$this->isPrivate, 'platform'=>$this->platform];
//        switch ($type) {
