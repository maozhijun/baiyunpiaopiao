...
class CommunityController extends Controller
{
    ...
    public function communities(Request $request) {
        ...
        return response()->json(['code'=>200, 'communities'=>$communities]);
    }
    ...
    public function focus(Request $request) {
        ...
        if ($type == CommunityMember::op_focus) {
            //判断是否关注
            if (isset($cm)) {
                return response()->json(['code'=>'403', 'msg'=>'您已关注过此社区。']);
            }
            ...
            if (!isset($cm)) {
                return response()->json(['code'=>'403', 'msg'=>'您尚未关注此社区。']);
            }
            $cm->delete();
            $msg = '取消关注成功';
        }
    ...
    public function topics(Request $request) {
        ...
        $page_size = $request->input('page_size', 15);//条数
        ...
        if (!isset($community)) {
            return response()->json(['code'=>404, 'msg'=>'社区不存在']);
        }
        ...
        $page = $query->paginate($page_size);
        $array = $page->items();
        $topics = [];
        foreach ($array as $topic) {
            $topics[] = $topic->topic2Json();
        }
        return response()->json(['code'=>200, 'topics'=>$topics]);
    }
